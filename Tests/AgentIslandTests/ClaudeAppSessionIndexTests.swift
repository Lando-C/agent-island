...
final class ClaudeAppSessionIndexTests: XCTestCase {
    func testNestedMetadataResolvesNewestDuplicate() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let older = root.appendingPathComponent("account/project/older.json")
        ...
        let result = ClaudeAppSessionIndex.load(roots: [root], matching: [sessionID])
        XCTAssertEqual(result[sessionID]?.title, "New")
        XCTAssertEqual(result[sessionID]?.sessionId, "local_new")
    }

    func testOriginalSessionWinsOverResumeImport() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let folder = root.appendingPathComponent("account/project", isDirectory: true)
        ...
        let result = ClaudeAppSessionIndex.load(roots: [root], matching: [sessionID])
        XCTAssertEqual(result[sessionID]?.sessionId, "local_6564d6f2-43c6-4c04-8739-0882b4728a07")
    }

    func testGenericTitleIncludesWorkspaceAndShortIdentity() {
        let info = ClaudeAppSessionInfo(
            sessionId: "local_1",
            cliSessionId: "70266e98-3300-4871-8ec4-8edba1ee8a24",
            cwd: "/Users/example/FDA2.0",
        ...
        XCTAssertEqual(ClaudeAppSessionIndex.displayTitle(for: info), "FDA2.0 · New session · 70266e98")
    }
