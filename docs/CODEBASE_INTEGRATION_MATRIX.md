...
| request_user_input | Structured pending questions and Claude/Codex supported-schema write-back | AgentBro `PendingQuestion`, `QuestionOption`; Mio AskUserQuestion/QuestionResponder references | Preserves choices, multi-select, Other, secret fields, and original question identity. | Keep Codex response schemas fail-closed until redacted live fixtures confirm each version. |
| Chat details | Shared incremental `ConversationStore` for Claude/Codex transcript, Hook, and broker events | Vibe `ChatHistoryManager.swift`, `ConversationParser.swift`, `ChatView.swift`, `ToolResultViews.swift`; AgentBro `TaskInfo`, `ToolResult` | Dialogue-first view; work payloads are compact and opt-in. | Add safe app/web provider adapters; never expose raw tool JSON as the default reading surface. |
...
| Smart suppress | `SmartSuppression.swift` plus app/browser/TTY/tmux/cmux target checks | Vibe `TerminalVisibilityDetector.swift`; DevIsland `isSessionFrontmost` | Suppresses only when a matching target can be established. | Add real-machine Warp/kitty/Kaku exact-pane regression coverage. |
...
| Sound | Optional local system sounds for start/done/attention | Vibe `SoundSelector.swift`; Mio `SoundManager.swift` | Uses the same state reducer as the island. | Add quiet hours, per-event selection, and custom-sound validation. |
