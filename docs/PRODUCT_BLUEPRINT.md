...
| App / Web / CLI 区分 | 不能把 App 工作误报成 CLI | 已用进程链和 terminal metadata 修正一部分 |
| Waiting / Approval / Done | 等待人推进不能显示异常；完成不能长期霸屏 | 已有 SessionStore + ExpansionController，仍需 PendingRequest |
| 旧会话收敛 | 超过 12h idle/waiting 不占列表 | 待做 |
| 僵尸检测 | pid/tmux pane/terminal 不存在时标 ended | 待做 |
...
| iTerm2 / Terminal | 按 tty/session 选 tab | 已接入 |
